# ArgusReach — Process Overview
## Send this to anyone who submits the email capture on the website

**Subject:** How ArgusReach works — your overview

Hi [First Name],

Thanks for your interest in ArgusReach. Here's exactly how we work — no fluff.

---

**The problem we solve**

Most financial advisors, insurance agencies, and professional practices are 80–100% dependent on referrals for new business. Referrals are great — until they're not. They're unpredictable, unscalable, and completely outside your control.

ArgusReach builds you a second engine: a consistent, proactive outbound system that runs in the background whether you think about it or not.

---

**What we actually do**

After one 45-minute onboarding call, here's what happens:

**1. We build your prospect list**
We research and source verified contacts that match your exact ideal client profile — right title, right company size, right geography, right industry. Clean data, no bounces.

**2. We write every email**
Every message is written specifically for the individual prospect — referencing their company, their role, their market context. Not a template. Not a mail merge. Something that reads like you wrote it yourself, because we wrote it to sound exactly like you.

**3. You approve before anything sends** *(optional)*
If you want full visibility, we send you every email for approval before it goes out. Nothing sends without your sign-off. Most clients start here, then switch to fully automated once they've seen the quality.

**4. We run the follow-up**
4–5 touches over three weeks. Most positive replies come after touch 3 or 4. We never stop too early.

**5. Hot replies come to you**
When a prospect responds with interest, we notify you immediately and route them to your booking link. You show up to the meeting. We handle everything before that.

**6. You get a monthly report**
Open rates, reply rates, meetings booked, what's working, what we're changing. Full transparency.

---

**What results look like**

Based on industry benchmarks for AI-personalized B2B cold email:

- Open rates: 35–55% (industry average: 15–20%)
- Positive reply rate: 4–8% of contacts reached
- Meetings booked: 4–12 per month depending on plan and vertical

These are projections, not guarantees. Performance depends on your offer, your vertical, and how clearly we can define your ideal prospect.

---

**Pricing**

| Plan | Monthly | Contacts |
|------|---------|----------|
| Starter | $750/mo | 200 contacts |
| Growth | $1,500/mo | 500 contacts |
| Scale | $2,500/mo | 1,000+ contacts |

All plans include a $500 one-time setup fee due at signing. Monthly billing begins when your campaign launches — not before. 3-month minimum from launch date. No contracts after that.

One new client typically pays for 6–12 months of ArgusReach. That's the math.

---

**Who we work with**

We specialize in:
- Registered Investment Advisors (RIAs) and wealth management firms
- Independent insurance agencies
- Physical therapy practices
- Mental health group practices
- Commercial real estate brokers
- Professional services (attorneys, CPAs, consultants)

We focus on Tampa Bay and Florida, with select engagements nationally.

---

**Next step**

If this sounds like it could fit, the best next move is a free 15-minute strategy call. No pitch — just a conversation about your business, your ideal client, and whether this makes sense for you.

Book here: https://calendly.com/vito-argusreach/30min

Or just reply to this email with any questions.

— Vito R.
ArgusReach
vito@argusreach.com

---
*ArgusReach · Tampa Bay, FL · argusreach.com*
*To unsubscribe, reply with "remove" and we'll take care of it immediately.*
