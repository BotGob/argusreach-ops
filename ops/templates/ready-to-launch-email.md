# Ready to Launch Email Template
> Send manually from vito@argusreach.com after:
> - DNS verified ✅
> - Warmup score ≥ 85% ✅
> - Sequence approved by client ✅
> - Calendar connected ✅
> This email collects subscription payment and is the final gate before launch.

---

**Subject:** Ready to launch - one last step

Hi [First Name],

Good news - everything is in place:

✅ Your outreach email is authenticated and warmed up
✅ Your prospect list is built and verified
✅ Your sequence is approved
✅ Your booking link is live

We're ready to launch. The only remaining step is your subscription payment to kick off your first month.

**[INSERT THEIR PLAN LINK - one link only, matches plan selected in intake]**
- Starter $750/mo → https://buy.stripe.com/5kQdR8b6IcST6VB1Jk5gc02
- Growth $1,500/mo → https://buy.stripe.com/14A3cu7UwbOP2Fl0Fg5gc00
- Scale $2,500/mo → https://buy.stripe.com/3cI9AS2AcdWXdjZcnY5gc03

Once payment is confirmed we'll activate your campaign and your first emails will go out within 24 hours.

— Vito Resciniti
Founder, ArgusReach
vito@argusreach.com

---
## Gob checklist before Vito sends this email
- [ ] DNS verified - SPF, DKIM, DMARC all passing (check mail-tester.com or dig)
- [ ] Warmup score ≥ 85% in Instantly
- [ ] Sequence approved by client
- [ ] Calendly event types created, client has connected their calendar
- [ ] Prospect list loaded in Instantly DRAFT campaign
